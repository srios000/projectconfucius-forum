...
export async function POST(req: NextRequest) {
    ...
    if (!communityId || !key.startsWith(`communities/${communityId}/banner/`)) {
    ...
    await db.update(communities).set({ bannerUrl: newUrl }).where(eq(communities.id, communityId));

    const oldKey = parseForumObjectKey(existing?.bannerUrl);
    if (oldKey && oldKey !== key) {
        deleteForumObject(oldKey).catch((err) => {
            console.error("[community-banner/confirm] best-effort delete failed", err);
        });
    }

    return NextResponse.json({ bannerUrl: newUrl });
}
